...
class DB extends require('./db'){
	...
	populateSilqoin(config, ready){
		
		let currency = 2**8
		let max = 40000
		let count = 0
		for(let i=0, s = 0; i<max; i++, s += currency){
			let owner = this.founder[i % this.founder.length]
			this.serial(s, h => {
	...
	ready(result){
		this.populateSilqoin(config, result => {
			this.populateCoinelis(config, result => {
